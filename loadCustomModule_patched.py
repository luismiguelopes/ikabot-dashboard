...
def loadCustomModule(session, event, stdin_fd, predetermined_input):
    ...
    while True:
        try:
            ...
            # int() cast: predetermined_input pops raw strings, but comparisons
            # and arithmetic below need an int.
            choice = int(read(min=0, max=len(modules) + 2, digit=True))
            ...
            if choice == 0:
                ...
                del_choice = int(read(min=0, max=len(modules) - 1, digit=True))
