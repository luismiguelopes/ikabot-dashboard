...
from espionage_parsers import (
    ...
    _parse_movement_datetime, parse_fleet_movements, enemy_fleet_clean_window_secs,
    _alt_to_resource_key, _RESOURCE_ALT_MAP,
)
...
def _dispatch_queue_items():
    try:
        ...
        return queue_items(SPY_DISPATCH_QUEUE)
    except Exception:
        logger.error("[espionage] leitura da fila de dispatch falhou", exc_info=True)
        return []


def _recall_queue_items():
    try:
        from db_manager import queue_items
        return queue_items(RECALL_QUEUE)
    except Exception:
        logger.error("[espionage] leitura da fila de recall falhou", exc_info=True)
        return []


def _queue_recall(item):
    """Enqueue a recall request in the shared SQLite queue. Returns True on success."""
    try:
        from db_manager import queue_add
        queue_add(RECALL_QUEUE, item)
        return True
    except Exception:
        logger.error("[espionage] enfileirar recall falhou", exc_info=True)
        return False
...
def has_pending_dispatch():
    ...
    try:
        return bool(_dispatch_queue_items())
    except Exception:
        return False
...
def process_dispatch_queue(session):
    ...
    # Remove exactly the items handled above; entries queued meanwhile stay untouched
    try:
        from db_manager import queue_remove
        queue_remove(SPY_DISPATCH_QUEUE, [it.get("id") for it in pending])
    except Exception:
        logger.error("[espionage] limpeza da fila de dispatch falhou", exc_info=True)
...
def has_due_recalls():
    ...
    try:
        ...
        return any(int(it.get("nextAttemptAfter", 0)) <= now
                   for it in _recall_queue_items())
    except Exception:
        return False


def _retry_recall_later(item):
    """Reschedule a failed recall attempt 3-10 min ahead instead of immediately.
    queue_add replaces by id, so the item is updated in place in the shared queue."""
    item = dict(item, nextAttemptAfter=int(time.time()) + random.randint(3, 10) * 60)
    try:
        from db_manager import queue_add
        queue_add(RECALL_QUEUE, item)
    except Exception:
        logger.error("[espionage] reagendar recall falhou", exc_info=True)


def _process_recall_queue(session):
    """Send retreat requests to the game server for all due pending recalls."""
    import ikabot.config as ikabot_config
    pending = _recall_queue_items()
    if not pending:
        return
    done_ids = []
    for item in pending:
    ...
    if done_ids:
        try:
            from db_manager import queue_remove
            queue_remove(RECALL_QUEUE, done_ids)
        except Exception:
            logger.error("[espionage] limpeza da fila de recall falhou", exc_info=True)
